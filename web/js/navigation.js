...
export function toggleOverlays() {
    ...
    DOM.overlaysBtn.textContent = state.showOverlays ? '👁️ Hide Overlays' : '👁️ Show Overlays';
}
...
export function toggleTranslations() {
    ...
    DOM.translationsBtn.textContent = state.showTranslations ? '🌐 Hide Translations' : '🌐 Show Translations';
}
...
export function filterManuals() {
    ...
    if (state.selectedTags.size > 0) {
        filteredManuals = filteredManuals.filter(manual => {
            ...
            return Array.from(state.selectedTags).every(tagId =>
                manual.tags.includes(tagId)
            );
        });
    }
...
export function toggleTagFilter(tagId) {
    ...
    document.querySelectorAll('.tag-filter').forEach(btn => {
        const btnTagId = Array.from(Object.entries(state.tagsData.tags))
            .find(([_, tag]) => tag.name === btn.textContent)?.[0];
